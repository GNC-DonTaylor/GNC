------------------------------
-- FUNCTION: FN_DEMANDROLLUP
-- 
-- FN_DEMANDROLLUP(ITEMROWID,NUMERIC OR [PY/CY/NY/NULL=PY+CY],[X/Y/Z/F/S/U/F1..U3/NULL=FSU/%=ALL],[HSFILTER/NOFILTER=NULL])
------------------------------
-- RETURNS THE DEMAND QTY FOR THE @LOTYEAR, @LOTSEASON PARAMETERS PASSED TO IT:
...
------------------------------
CREATE OR ALTER   FUNCTION [DBO].[FN_DEMANDROLLUP]
	...
	BEGIN
		IF @LOTSEASON = 'FSU'
			BEGIN --MULTIPLE LOTSEASON CALCULATIONS--FSU with Wildcards
				...
				(
					SELECT 
						DBO.FN_DEMANDROLLUP(@ITMROWID,@LOTYEAR,'F%',@HSFILTER) +
						DBO.FN_DEMANDROLLUP(@ITMROWID,@LOTYEAR,'S%',@HSFILTER) +
						DBO.FN_DEMANDROLLUP(@ITMROWID,@LOTYEAR,'U%',@HSFILTER)
				)
			END
		ELSE
					...
					BEGIN --NUMERIC VALUE PASSED AS @LOTYEAR
						IF @LOTYEAR <= @CY
							...
							( --COMBINED CURRENT/PREVIOUS YEARS
								SELECT	SUM(DBO.FN_DEMAND(@ITMROWID, VLOT.ROWID)) 
