------------------------------ 
-- FUNCTION: FN_DATE2WEEK 
------------------------------ 
-- RETURNS THE REPORT WEEK FROM THE DATE PARAMETER PASSED TO IT 
...
------------------------------ 
CREATE OR ALTER FUNCTION [DBO].[FN_DATE2WEEK] 
( @INDATE DATETIME 
) RETURNS INTEGER 
AS 
BEGIN 
...
--FLOOR((@WEEKCOUNT - 1)/4) + 1 AS PERIOD 
	...
	BEGIN 
		...
		-- ARBITRARY RULE: WEEKOFYEAR 53 TRANSLATES TO WEEK 5 
		IF @WEEKCOUNT = 53 --FORCE RESULT FROM PERIOD 14/WEEK 1 TO PERIOD 13/WEEK 5 
			SET @RESULT = 5 
		ELSE -- Calculate Week of Period (1-4) 
			SET @RESULT = ((@WEEKCOUNT - 1) % 4) + 1 
	END 
	RETURN @RESULT 
END 
