------------------------------
-- FUNCTION: FN_SALESNOTEROLLUP
-- 
-- FN_SALESNOTEROLLUP(ITEMROWID,[PY/CY/NY/NULL=PY+CY/FY/%=ALL],[X/Y/Z/F/S/U/F1..U1/NULL=FSU/%=ALL],[HSFILTER/NOFILTER=NULL])
------------------------------
...
------------------------------
------------------------------
CREATE OR ALTER   FUNCTION [DBO].[FN_SALESNOTEROLLUP]
		...
		IF @LOTSEASON = 'FSU'
			BEGIN --MULTIPLE LOTSEASON CALCULATIONS--FSU with Wildcards
				IF @LOTYEAR = 'PY'
					SET @RESULT = 
					( --PREVIOUS YEAR
						SELECT	TOP 1 DBO.FN_SALESNOTE(@ITMROWID, LOT.ROWID, NULL) 
						FROM	IMLOT LOT (NOLOCK) 
						WHERE	LOT.R_ITEM = @ITMROWID 
						AND		DBO.FN_SALESNOTE(@ITMROWID, LOT.ROWID, NULL) IS NOT NULL
