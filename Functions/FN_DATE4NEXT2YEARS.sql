------------------------------ 
-- FUNCTION: FN_DATE4NEXT2YEARS 
--	CREATED FOR THE VW_RPT_QTY_SHIPPED_3YR SO WE DON'T HAVE TO CALL FN_DATE4NEXTYEAR(FN_DATE4NEXTYEAR([DATE])) 
------------------------------ 
-- RETURNS THE EQUIVALENT REPORT DATE FOR NEXT 2 YEARS FROM THE DATE PARAMETER PASSED TO IT 
-- NULL DATE ASSUMES CURRENT DATE USING GETDATE() 
------------------------------ 
-- DON TAYLOR @ 10/14/2021 
------------------------------ 
-- EMBEDDED FN_YEAR2DATE(), FN_DATE2YEAR(), FN_DATE2PERIOD() & FN_DATE2WEEK() LOGIC TO REDUCE FUNCTION DAISY CHAINS @10/14/21-DT 
------------------------------ 
CREATE OR ALTER FUNCTION [DBO].[FN_DATE4NEXT2YEARS] 
( @INDATE DATETIME 
) RETURNS DATETIME 
AS 
BEGIN 
	...
	BEGIN 
		...
		--FN_YEAR2DATE()+1 CODE: (WE WANT FIRST DAY OF NEXT YEAR) 
			...
			SET @STARTDATE = '08/01/' + TRIM(STR(@YEAR+1))							--AUG 1ST DATE OF NEXT 2 YEARS 
...
--		--FIND 1ST DAY OF NEXT 2 YEARS 
--		SET @STARTDATE = DBO.FN_YEAR2DATE(@YEAR + 2) 
