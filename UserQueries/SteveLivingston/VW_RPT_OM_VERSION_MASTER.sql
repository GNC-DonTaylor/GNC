...
/****** Object:  View [dbo].[VW_RPT_OM_VERSION_MASTER]    Script Date: 10/24/2023 11:41:23 AM ******/
SET ANSI_NULLS ON
GO
...
(SELECT PROFILECODE FROM OMTRANSACTIONDP WHERE ROWID = OH.R_PROFILE) AS PROFILECODE, 
OH.STATUS,
...
ISNULL(OD.UNITPRICE + ISNULL(OD.HANDLINGCHARGEPERITEM, 0) + ISNULL(OD.TAGGINGCHARGEPERITEM, 0) + ISNULL(OD.FREIGHTRATEPERITEM, 0),0) AS LANDEDPRICE,
--EXTENDED PRICES
...
(ISNULL(OD.UNITPRICE + ISNULL(OD.HANDLINGCHARGEPERITEM, 0) + ISNULL(OD.TAGGINGCHARGEPERITEM, 0) + ISNULL(OD.FREIGHTRATEPERITEM, 0),0)) * IIF(CHARINDEX(OH.STAGE,'A;S;C')>0,ISNULL(OD.QUANTITYSHIPPED,0),ISNULL(OD.QUANTITYORDERED,0)) AS EXTLANDEDPRICE,
--FREIGHT BREAKDOWN
