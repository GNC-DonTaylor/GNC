/*
--Inventory by Item
--Inventory by Item - Page Break
SELECT *
FROM VW_RPT_INVENTORY_BY_ITEM
WHERE 1=1
--
--Quantity Counts by Location
SELECT *
FROM VW_RPT_INVENTORY_BY_ITEM
--
--Inventory by Item DAILY
SELECT * 
FROM VW_RPT_INVENTORY_BY_ITEM 
WHERE HSSTART = CAST (GETDATE()-1 AS DATE) 
...
CREATE OR ALTER VIEW [dbo].[VW_RPT_INVENTORY_BY_ITEM]
AS
SELECT
ROWID, 
COMPANYID, SHIPPERCODE, 
...
INVENTORYNOTE, 
SNSTART, SALESNOTE, 
PENDINGRECEIPTS, BACKORDERRECEIPTS, 
INRECEIVING, 
...
FUTUREEVAL, 
LOCATIONNOTE, LOCNOTEDATE, 
BYPASSLOCATION, LOCATIONPTN1, LOCATIONPTN2, 
PTNOTE1, MCTYPE, MCSTATUS, SORT
FROM
(
	SELECT
		IMITEM.ROWID , IMITEM.COMPANYID, WH.IDENTITYID AS SHIPPERCODE,IMITEM.ITEMCODE,LOC.CLASSCODE AS ALPHA,
		PC.PRODUCTCATEGORYCODE  AS PLANTGROUP,
		IMITEM.REFERENCE_5 AS SORTNAMEVARIETY, CNT.CONTAINERSORT,
	...
	FROM IMITEM 
	JOIN IMCONTAINER CNT (NOLOCK) ON CNT.ROWID = IMITEM.R_CONTAINERCODE
	LEFT JOIN VW_IMLOTVIEW2 LOT (NOLOCK) ON LOT.R_ITEM= IMITEM.ROWID
	LEFT JOIN OMREVIEWMASTERCODES MC (NOLOCK) ON MC.R_LOT=LOT.ROWID
	LEFT JOIN IMITEMDP DP (NOLOCK) ON DP.ROWID = IMITEM.R_PROFILE
	...
	LEFT JOIN IMCODES CI (NOLOCK) ON CI.CODE = IMITEM.USERDEFINEDCODE_1 AND CI.CODETYPE = '11'
	LEFT JOIN IDMASTER WH (NOLOCK) ON WH.ROWID = IMITEM.R_WAREHOUSE
	LEFT JOIN IMPRODUCTCATEGORY PC (NOLOCK) ON PC.ROWID =IMITEM.R_PRODUCTCATEGORY
	...
	JOIN IMITEM (NOLOCK) ON IMITEM.ROWID = OMREVIEWMASTERCODES.R_ITEM
	JOIN IMCONTAINER CNT (NOLOCK) ON CNT.ROWID = IMITEM.R_CONTAINERCODE
	JOIN VW_IMLOTVIEW2 LOT (NOLOCK) ON LOT.ROWID = OMREVIEWMASTERCODES.R_LOT  --ITEMIMITEM.ROWID
	--LEFT JOIN OMREVIEWMASTERCODES MC (NOLOCK) ON MC.R_LOT=LOT.ROWID
	LEFT JOIN IMITEMDP DP (NOLOCK) ON DP.ROWID = IMITEM.R_PROFILE
