...
--46 AVAIL BY LOCATION STATIC
SELECT *
FROM VW_RPT_AVAILABILITY_BY_LOCATION_STATIC (NOLOCK)
WHERE RECORDTYPE <> 'P' 
--------------------------------- 
AND ITEMCODE LIKE '001993.031.1'
*/
CREATE OR ALTER VIEW [DBO].[VW_RPT_AVAILABILITY_BY_LOCATION_STATIC]
AS
...
SELECT
	*,
	...
	--TEST SEASON:
	DBO.FN_SUPPLYROLLUP(ITEMROWID, NULL, NULL, NULL, NULL) AS SUPPLYX,
	DBO.FN_DEMANDROLLUP(ITEMROWID, NULL, NULL, NULL) AS DEMANDX
FROM	(
		SELECT
			CO.COMPANYID,
...
--NOW USING VW_RPT_SEASONCODE_GNC
			FROM VW_RPT_SEASONCODE_GNC SEA0 (NOLOCK) 
			JOIN VW_RPT_SEASONCODE_GNC SEA1 (NOLOCK) ON SEA0.ENDDATE <= SEA1.STARTDATE
			JOIN VW_RPT_SEASONCODE_GNC SEA2 (NOLOCK) ON SEA1.ENDDATE <= SEA2.STARTDATE
			-----------------WHERE SEA0.ENDDATE >= GETDATE() 
			WHERE SEA0.ENDDATE >= ('01/01/'+trim(str(dbo.FN_SALEYEAR(null))))	--USE STATIC DATE FOR SPRING OF CURRENT YEAR.
			ORDER BY SEA0.SEASONCODE,SEA1.SEASONCODE,SEA2.SEASONCODE
