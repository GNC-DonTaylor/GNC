/* GP_LABORGROUP_SHIPPING_39PERIOD
--RT:5:15 @ 52,871
--GP LaborGrp Ship39P-Det EXCEL
--GP LaborGrp Ship39P-Sum EXCEL

SELECT 
* 
FROM VW_RPT_GP_LABORGROUP_SHIPPING_39PERIOD (NOLOCK)
--MACRO-------------
...
CREATE OR ALTER VIEW [dbo].[VW_RPT_GP_LABORGROUP_SHIPPING_39PERIOD] 
AS 
SELECT 
...
----------------------------------------
					...
					LEFT JOIN (	
								...
								FROM (
										SELECT 
											WH.IDENTITYID AS WAREHOUSEID, 	
											DBO.FN_DATE2YEAR(OH.INVOICEDATE) AS YEAR, 
											DBO.FN_DATE2PERIOD(OH.INVOICEDATE) AS PERIOD, 
											THRU.DATE AS THRUDATE, 
											SUM(ISNULL(OD.QUANTITYSHIPPED,0)) AS UNITS, 
											SUM(ISNULL(OD.QUANTITYSHIPPED,0)*(ISNULL(CNT.VOLUME,.01)/615.99)) AS EQUIVUNITS
										...
										AND OH.INVOICEDATE <= THRU.DATE
										GROUP BY WH.IDENTITYID, DBO.FN_DATE2YEAR(OH.INVOICEDATE), DBO.FN_DATE2PERIOD(OH.INVOICEDATE), THRU.DATE
								) U 
