...
--UPDATED SALESREP ON THIS MODIFIED VERSION TO IGNORE SALESREP OVERRIDES AT THE INVOICE LEVEL. I.E. OH.SALESPERSON_1. PER DJ 2/7/2025
--
--UPDATED FREIGHT CALC: ALLOW ALL TRANSACTIONTYPES AND REVERSE SIGN ON CREDITS/DEBITS. PER DJ 2/7/2025
...
CREATE OR ALTER VIEW [DBO].[VW_RPT_SQL_ACTUALS_MODIFIED]
AS
--1.1 Shipments
SELECT
1 AS REPORTGROUP,
...
ISNULL(FRT.AMOUNT,0)*IIF(OH.TRANSACTIONTYPE= 'I', 1, -1) AS AMOUNT	--REVERSE THE SIGN FOR ADJUSTMENTS AND CREDITS
FROM OMTRANSACTIONHEADER OH (NOLOCK)
--JOIN OMTRANSACTIONDETAIL OD (NOLOCK) ON OD.R_TRANSACTIONHEADER = OH.ROWID
...
JOIN IDCODES CTYP (NOLOCK) ON CTYP.CODE = ICONS.USERDEFINEDCODE_3 AND CTYP.CODETYPE = '74'
WHERE OH.RECORDTYPE='Z' --COMMITTED RECORDS ONLY
--SPEC HAS CHANGED TO ALLOW ALL TRANSACTIONTYPES NOW. 2/6/2025 PER DJ
--AND OH.TRANSACTIONTYPE='I' --ORDERS/INVOICES ONLY
