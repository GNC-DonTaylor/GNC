/*
--86 LEFT TO SHIP
--104 OPEN ORDERS LEFT TO SHIP 
--104 OPEN ORDERS LEFT TO SHIP EXCEL
SELECT * 
FROM VW_RPT_LEFT_TO_SHIP (NOLOCK) 
WHERE VW_RPT_LEFT_TO_SHIP.RECORDTYPE <> 'P'
and warehouseid = '20'--'40'--61525'--'40-64597-9'
--and CONSIGNEEID = '509561'--
--AND customerid = '10043'--'54840'--'829045'

and step not in ('preeval','reserve')
and natlacct = 'n'
and salesrep = 's15'
*/
CREATE OR ALTER VIEW [DBO].[VW_RPT_LEFT_TO_SHIP]
AS
SELECT 
CO.COMPANYID, 	
...
--TRIM(OH.FMTRIPNUMBER)+'/'+ISNULL(TRIM(STR(OH.FMPUSTOPNUMBER)),'') AS TRIPSTOPNO,
TRIM(FH.TRIPNO)+'/'+ISNULL(TRIM(STR(FD.DELSTOPNO)),'') AS TRIPSTOPNO,
IIF(FH.TRIPNO IS NULL, 'Y', 'N') AS OPENORDER_YN,
...
--THIS ARGOS FUNCTION NO LONGER INCLUDES VOLUME CALCULATION RESULTS:
--[DBO].[FNOM_CALCULATETRANSTOTALS](OH.ROWID,'OUTTOTALVOLUME') AS TOTCNTVOLUME,
--GETTING VOLUME FROM THE DISPATCH OBJECT NOW:
ISNULL(OBJ.CUBESORDERED,0) AS TOTCNTVOLUME, 

----------------------------------------------------------------------
--TEST TO INCLUDE TRIP VOLUME INCASE THIS IS ACTUALLY WHAT THEY WANT--
...
WHERE H.ROWID =FH.ROWID )AS TOTTRIPVOLUME,
----------------------------------------------------------------------
