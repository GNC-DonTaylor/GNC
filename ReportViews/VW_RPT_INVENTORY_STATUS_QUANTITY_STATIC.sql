/*
--71 Inventory Status - Quantity (STATIC)	RUNTIME 0:44
SELECT *
FROM VW_RPT_INVENTORY_STATUS_QUANTITY_STATIC (NOLOCK)
WHERE BUCKETTOTAL <> 0
and warehouseid = '20'

ORDER BY WAREHOUSEID, PLANTGROUP, SORTNAME, CONTAINERSORT, QUALITY
*/
CREATE OR ALTER VIEW [DBO].[VW_RPT_INVENTORY_STATUS_QUANTITY_STATIC]
AS
...
--NOW USING VW_RPT_SEASONCODE_GNC 
			JOIN VW_RPT_SEASONCODE_GNC SEA (NOLOCK) ON SUBSTRING(SEA.SEASONCODE,4,2) = WH.IDENTITYID
--			AND GETDATE() BETWEEN SEA.STARTDATE AND SEA.ENDDATE
			AND ('01/01/'+trim(str(dbo.FN_SALEYEAR(null)))) BETWEEN SEA.STARTDATE AND SEA.ENDDATE	--USE STATIC DATE FOR SPRING OF CURRENT YEAR.
