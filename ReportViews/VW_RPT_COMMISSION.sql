/*
saving off a formula that I am removing from the report but don't want to forget it just yet:
-------------------------------------------------------
sumSum([CURRPAYMENT]) -
Iif([FREIGHTTOTAL]*[REVSIGN] > sumSum([PREVPAYMENT]*[REVSIGN])
, [FREIGHTTOTAL] - sumSum([PREVPAYMENT])
, 0)
-------------------------------------------------------


--130 SALES COMMISSION REPORT 
SELECT * 
FROM VW_RPT_COMMISSION (NOLOCK) 
WHERE NATIONALACCOUNT <> 'Y' 
AND CUSTOMERGROUP IS NULL 
AND WAREHOUSEID IN ('10','20','40')
AND SALESREPID IS NOT NULL
-----
and warehouseid = '10'
and salesrepid = 'S21'
and customerid = '10019'

--NATL ACCT COMMISSION REPORT (MANAGER VERSION USES VW_RPT_COMMISSION_MGR VIEW)
SELECT * 
FROM VW_RPT_COMMISSION (NOLOCK) 
WHERE NATIONALACCOUNT = 'Y'
AND WAREHOUSEID IN ('10','20','40') 
AND SALESREPID IS NOT NULL 
and warehouseid = '10'
and salesrepid = 'S21'--'S04'
and customerid = '10019'--'823560'

---------TESTING CRITERIA:
AND CUSTOMERGROUP = '33'
AND CONVERT (date, PAYMENTDATE) = '07-13-2021'
...
ORDER BY 
	...
	TRANSACTIONTYPE
*/
CREATE OR ALTER VIEW [DBO].[VW_RPT_COMMISSION]
AS
SELECT
	X.*,
...
FROM ( 
		...
		---
			...
			--ADDED TO INCLUDE CUSTOMERSTATE:
...
--					(CD.AMOUNTAPPLIED + CD.DISCOUNTTAKEN) AS CURRPAYMENT
		...
		---ADDED TO DETERMINE CUSTOMER STATE
