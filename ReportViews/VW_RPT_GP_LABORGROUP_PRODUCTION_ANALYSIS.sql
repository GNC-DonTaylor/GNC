/* GP_LABORGROUP_PRODUCTION_ANALYSIS
--RT:1:22 @ 13,451
--GP LaborGrp ProdAnal-Det EXCEL
--GP LaborGrp ProdAnal-Sum EXCEL

SELECT 
* 
FROM VW_RPT_GP_LABORGROUP_PRODUCTION_ANALYSIS (NOLOCK)
--MACRO-------------
...
CREATE OR ALTER VIEW [dbo].[VW_RPT_GP_LABORGROUP_PRODUCTION_ANALYSIS] 
AS 
			...
			FROM ( 
					...
					SELECT 
						...
						(	SELECT SUM(ISNULL(PRD.RECEIVEDQUANTITY,0))
							...
							AND PRH.RECEIPTTYPE = 'P'	--PO RECEIPT
							AND WH.IDENTITYID = WAREHOUSE 
							AND PRH.COMMITDATE >= DBO.FN_YEAR2DATE(DBO.FN_DATE2YEAR(GETDATE()))		--START-OF-CURRENTYEAR
							AND PRH.COMMITDATE <= (	GETDATE() -					--TODAY
													(DATEPART(DW,GETDATE())%7)	--PREV SATURDAY
													)
							), 0)  AS CY_UNITS, 
					...
					SELECT 
						...
						(	SELECT SUM(ISNULL(PRD.RECEIVEDQUANTITY,0))
							...
							AND PRH.RECEIPTTYPE = 'P'	--PO RECEIPT
							AND WH.IDENTITYID = WAREHOUSE 
							AND PRH.COMMITDATE >= DBO.FN_YEAR2DATE(DBO.FN_DATE2YEAR(GETDATE())-1)	--START-OF-PREVIOUSYEAR
							AND PRH.COMMITDATE <= (	GETDATE() -											--TODAY
													DBO.FN_YEAR2DATE(DBO.FN_DATE2YEAR(GETDATE())) +		--LESS START-OF-CURRENTYEAR
													DBO.FN_YEAR2DATE(DBO.FN_DATE2YEAR(GETDATE())-1) -	--PLUS START-OF-PREVIOUSYEAR
													(DATEPART(DW,GETDATE())%7)							--PREV SATURDAY
